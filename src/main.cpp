...
uint256 hashGenesisBlock("0x9954c30955ee8f14483bc1ad2d87ced00bc0bd613ee80418a7bade6f6285440e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // OilGasCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "OilGasCoin Signed Message:\n";
...
{
    // OilGasCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // OilGasCoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // OilGasCoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // OilGasCoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // OilGasCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // OilGasCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // OilGasCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x9954c30955ee8f14483bc1ad2d87ced00bc0bd613ee80418a7bade6f6285440e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=9954c30955ee8f14483b, PoW=000002434666397ff1ee, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=b950714009, nTime=1392879759, nBits=1e0ffff0, nNonce=43795, vtx=1)
        //   CTransaction(hash=b950714009, ver=1, vin.size=1, vout.size=1, nLockTime=0)
        //     CTxIn(COutPoint(0000000000, 4294967295), coinbase 04ffff001d0104404e592054696d65732032302f4662722f32303134205374657665204a6f62732c204170706c65e280997320566973696f6e6172792c2044696573206174203536)
        //     CTxOut(nValue=50.00000000, scriptPubKey=0)
        //   vMerkleTree: 97ddfbbae6

        // Genesis block
        const char* pszTimestamp = "NY Times 20/Fbr/2014 Steve Jobs, Apple’s Visionary, Dies at 56";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xb95071400926ee842953709e40b26d25980273636d3ee82c7ba7a53be0aa9e9f"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // OilGasCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("OilgascoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("OilgascoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static OilgascoinMiner(CWallet *pwallet)
{
    printf("OilgascoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("OilGasCoin-miner");
    ...
    try { loop {
        ...
        printf("Running OilgascoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("OilgascoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&OilgascoinMiner, pwallet));
}
